...
lrt_filt_2PC <- merge(lrt_filt_2PC, new_chr_name, by.x = "Chromosome", by.y = "NCBI", all = TRUE)
...
sig_snps <- df_2PC %>% arrange(-LRT) %>% subset(log10p > -log10(0.05/nrow(df_2PC)))
write.csv(sig_snps, file = "RAD_2PC_sig_snps.csv")
```

```{r p-values & plotting - 2PC - filtered, echo=FALSE, fig.height=4, fig.width=14}
color_group <- rep(c("lightgrey", "black"), length.out = 29)
