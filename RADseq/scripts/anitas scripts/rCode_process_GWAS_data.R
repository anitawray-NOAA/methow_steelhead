...
packages <- c('GENESIS', 'SeqArray', 'SNPRelate', 'vcfR', 'GWASTools', 'tidyverse',
              'scales', 'reshape2', 'ASRgenomics', 'qqman')
...
gds_genoReader <- GdsGenotypeReader("gds", autosomeCode=1:29L, XchromCode = 35L, YchromCode = 35L, XYchromCode = 35L, MchromCode= 35L)
...
write.csv(phenos, file = 'complete_metadata_methow_steelhead_RADseq.csv', row.names = F)
...
growth <- (log(as.numeric(phenosOut2$WT)) - log(phenosOut2$Weight))/days
...
mort_2_day_GWAS$chr<-as.numeric(growth_GWAS$chr)

manhattan(mort_2_day_GWAS,chr="chr",bp="pos",p="Score.pval",snp="pos",logp=TRUE,ylim=c(0,15),
          suggestiveline=FALSE, genomewideline=-log10(0.01/60747))
dev.off()
...
mort_14_day_GWAS$chr<-as.numeric(growth_GWAS$chr)

manhattan(mort_14_day_GWAS,chr="chr",bp="pos",p="Score.pval",snp="pos",logp=TRUE,ylim=c(0,15),
          suggestiveline=FALSE, genomewideline=-log10(0.01/60747))
dev.off()
...
# Heritability
varCompCI(smolt_bin_null_mod, prop = TRUE)
# manhattan plot

jpeg('smoltification_GWAS.jpg', width = 1800, height = 900, units = "px", pointsize = 12)
s$chr<-as.numeric(growth_GWAS$chr)

manhattan(s,chr="chr",bp="pos",p="Score.pval",snp="pos",logp=TRUE,ylim=c(0,15),
          suggestiveline=FALSE, genomewideline=-log10(0.01/60747))
dev.off()
...
growth_GWAS$chr<-as.numeric(growth_GWAS$chr)

manhattan(growth_GWAS,chr="chr",bp="pos",p="Score.pval",snp="pos",logp=TRUE,ylim=c(0,15),
          suggestiveline=FALSE, genomewideline=-log10(0.01/60747), 
          annotatePval = 0.01/60747)
dev.off()
#dev.off()

qq(growth_GWAS$Score.pval)
